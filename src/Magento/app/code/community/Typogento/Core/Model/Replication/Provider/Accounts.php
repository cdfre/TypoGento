...
class Typogento_Core_Model_Replication_Provider_Accounts extends Typogento_Core_Model_Replication_Provider_Abstract {
	...
	protected function _update(Mage_Core_Model_Abstract $source, Mage_Core_Model_Abstract $target) {
		...
		if ($source instanceof Mage_Customer_Model_Customer) {
			// update frontend user
			$this->_updateFrontendUser($source, $target);
		} else if($source instanceof Typogento_Core_Model_Typo3_Frontend_User) {
			// update frontend user
			$this->_updateCustomer($source, $target);
		}
	}
	...
	protected function _assertSourceType(Mage_Core_Model_Abstract $object) {
		if ($this->_id === self::PROVIDER_ID_MAGENTO_CUSTOMER) {
			...
			if (!($object instanceof Typogento_Core_Model_Typo3_Frontend_User)) {
				throw new InvalidArgumentException();
			}
		}
	}
	...
	protected function _updateFrontendUser(Mage_Customer_Model_Customer $source, Typogento_Core_Model_Typo3_Frontend_User $target) {
		// validate customer
	...
	protected function _updateCustomer(Typogento_Core_Model_Typo3_Frontend_User $source, Mage_Customer_Model_Customer $target) {
		//
