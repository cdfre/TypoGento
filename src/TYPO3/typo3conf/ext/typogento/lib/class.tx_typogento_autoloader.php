 ...
 */
class tx_typogento_autoloader implements t3lib_Singleton {
	
	/**
	 * @var tx_typogento_magentoHelper
	 */
	protected static $_helper = null;
	
	/**
	 * Constructor for tx_typogento_interface
	 *
	 */
	public function __construct() {
		// init confguration helper if not already done
		if (!isset(self::$_helper)) {
			self::$_helper = t3lib_div::makeInstance('tx_typogento_magentoHelper');
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/typogento/lib/class.tx_typogento_autoloader.php']) {
	include_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/typogento/lib/class.tx_typogento_autoloader.php']);
}
