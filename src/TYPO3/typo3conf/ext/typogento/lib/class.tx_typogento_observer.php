 ...
 */
class tx_typogento_observer implements t3lib_Singleton, tslib_content_getDataHook {
	...
	public function renderPreProcess($params, t3lib_pagerenderer &$pObj) {
		...
		try {
			...
			$header = t3lib_div::makeInstance('tx_typogento_header');
	...
	public function configArrayPostProc($params, tslib_fe &$pObj) {
		...
		//if (!isset($pObj->config['tx_typogento'])) {
		try {
			t3lib_div::makeInstance('tx_typogento_router');
		} catch(Exception $e) {
			tx_typogento_div::throwException('lib_initalizing_routing_system_failed_error', 
				array($_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']), $e
			);
		}
		//}
	...
	public function getDataExtension($getDataString, array $fields, $sectionValue, $returnValue, tslib_cObj &$parentObject) {
		...
		|| strpos($sectionValue, 'tx_typogento.') === false) {
			return $returnValue;
		}
		// check if register:tx_typogento.page.<field> is used
		if (strpos($sectionValue, 'tx_typogento.page.') !== false) {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/typogento/lib/class.tx_typogento_observer.php']) {
