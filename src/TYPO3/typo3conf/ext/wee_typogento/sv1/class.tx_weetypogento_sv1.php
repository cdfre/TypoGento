 ...
 */
class tx_weetypogento_sv1 extends tx_sv_auth {
	
	public $prefixId = 'tx_weetypogento_sv1';
	
	public $scriptRelPath = 'sv1/class.tx_weetypogento_sv1.php';
	...
	public function authUser($user) {
		...
		if ($this->getLastError() !== true 
			|| !isset($user['tx_weetypogento_customer'])) {
			return $code;
		}
		// perform authentication
		try {
			// load costumer
			$customer = $this->_getCustomer()->load($user['tx_weetypogento_customer']);
	...
	public function postUserLookUp($params, &$pObj) {
		...
		try {
			...
			$session->loginById($pObj->user['tx_weetypogento_customer']);
		} catch (Exception $e) {
			$e = tx_weetypogento_div::exception('lib_sso_autologin_failed_error',
				array($pObj->user['uid'], $pObj->user['email'], $pObj->user['tx_weetypogento_customer']), $e
			);
			// @todo log exception
		}
	}
