 ...
 *
 */
class tx_weetypogento_statusReport implements tx_reports_StatusProvider {
	
	/**
	 * Constructor for class tx_weetypogento_statusReport
	 */
	public function __construct() {
		$GLOBALS['LANG']->includeLLFile('EXT:wee_typogento/reports/locallang.xml');
	}
	...
	protected function _getConnectorStatus() {
		...
		try {
			...
			if (Mage::helper('core')->isModuleEnabled('Wee_Typogento') === false) {
	...
	protected function _getApiStatus() {
		...
		try {
			...
			t3lib_div::makeInstance('tx_weetypogento_soapinterface')->isAvailable();
		} catch (Exception $e) {
			$message = $this->_renderFlashMessage($e->getMessage(), t3lib_FlashMessage::WARNING);
		}
		//
